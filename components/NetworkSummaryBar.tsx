...
export const NetworkSummaryBar: React.FC<NetworkSummaryBarProps> = ({ switches }) => {
  ...
  return (
    <div className="bg-white dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800 rounded-xl p-8 shadow-sm mb-8 relative overflow-hidden">
      {/* Decorative Accent */}
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-6 relative z-10">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-zinc-400 mt-1 font-medium italic">Sentralisasi manajemen infrastruktur IT & monitoring real-time</p>
        </div>
        <div className="flex items-center gap-4 bg-blue-50/50 dark:bg-blue-500/10 border border-blue-100 dark:border-blue-900/30 rounded-xl px-8 py-5 shadow-sm">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-5 mb-10">
        {[
          ...
          <div key={item.label} className="bg-slate-50/50 dark:bg-zinc-900/50 border border-slate-100 dark:border-zinc-800 p-5 rounded-xl flex items-center gap-4 transition-all hover:bg-white dark:hover:bg-slate-800 hover:shadow-md hover:border-slate-200 dark:hover:border-slate-700">
            <div className={`w-11 h-11 ${item.bg} rounded-xl flex items-center justify-center ${item.color}`}>
              <item.icon size={20} />
            </div>
            <div>
              <div className="text-[10px] text-slate-500 dark:text-zinc-500 font-bold uppercase tracking-widest leading-none mb-1.5">{item.label}</div>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-4">
          {topTypes.map(([type, count]) => {
            ...
            return (
              <div key={type} className="group bg-white dark:bg-zinc-900 border border-slate-100 dark:border-zinc-800 rounded-xl p-4 flex flex-col items-center gap-3 hover:bg-slate-50 dark:hover:bg-slate-800 transition-all cursor-default hover:shadow-lg hover:shadow-blue-500/5">
                <div className="w-10 h-10 bg-slate-100 dark:bg-zinc-800 group-hover:bg-blue-500 group-hover:text-white rounded-xl flex items-center justify-center text-slate-500 dark:text-zinc-400 transition-all duration-300 transform group-hover:scale-110">
                  <Icon size={18} />
                </div>
                <div className="text-center w-full">
                  <div className="text-xl font-black text-slate-900 dark:text-white leading-none mb-1.5">{count}</div>
                  <div className="text-[9px] text-slate-400 dark:text-zinc-500 font-black uppercase tracking-wider truncate px-1">{type}</div>
                </div>
              </div>
            );
          })}
