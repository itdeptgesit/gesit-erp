...
export function exportAssetHandoverBAST(
  asset: ITAsset,
  info: {
  ...
  doc.setFontSize(15);
  doc.setTextColor(15, 23, 42); // Slate-900
  doc.text('BERITA ACARA SERAH TERIMA ASET (BAST)', pageW / 2, currentY, { align: 'center' });

  // Generate dynamic doc number based on asset ID
  const now = new Date(info.handoverDate || Date.now());
  const year = now.getFullYear();
  const month = String(now.getMonth() + 1).padStart(2, '0');
  const docNumber = `BAST/IT-ASSET/TGC/${year}/${month}/${asset.assetId}`;
  ...
  // 4. Opening
  const openingText = `Pada hari ini, ${formatIndonesianLongDate(info.handoverDate)}, kami yang bertanda tangan di bawah ini telah melakukan serah terima barang inventaris kantor:`;
  const splitOpening = doc.splitTextToSize(openingText, contentW);
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(9.5);
  doc.setTextColor(15, 23, 42);
  ...
  drawPartyBlock(
    'PIHAK PERTAMA (Yang Menyerahkan)',
    info.itSupportName || 'IT Support Team',
    'Information Technology (IT)',
    '-',
    'PEMBERI TUGAS / IT DEPT',
    currentY
  ...
  // 6. Transfer Statement
  const statementText = 'Pihak Pertama menyerahkan 1 (satu) unit aset inventaris perusahaan kepada Pihak Kedua dalam kondisi prima dan siap digunakan untuk kepentingan operasional, dengan spesifikasi sebagai berikut:';
  const splitStatement = doc.splitTextToSize(statementText, contentW);
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(9.5);
  doc.setTextColor(15, 23, 42);
  ...
  // 7. Specifications / Asset Details Table
  const tableY = currentY;
  const colW = [10, 55, 30, 45, 30]; // 170
  const colX = [
    marginL,
    marginL + colW[0],
    marginL + colW[0] + colW[1],
  ...
  const rawItem = asset.item || 'IT Asset Device';
  const cleanItem = rawItem.length > 28 ? rawItem.substring(0, 26) + '...' : rawItem;
  doc.setFont('helvetica', 'bold');
  doc.text(cleanItem, colX[1] + 3, rowY + 6);
  
  doc.setFont('helvetica', 'normal');
  doc.text(asset.category || 'Laptop', colX[2] + 3, rowY + 6);
  
  doc.setFont('helvetica', 'bold');
  doc.text(asset.serialNumber || '-', colX[3] + 3, rowY + 6);
  
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(16, 185, 129); // Emerald-500
  doc.text(asset.condition || 'New', colX[4] + colW[4] / 2, rowY + 6, { align: 'center' });
  ...
  doc.setFontSize(9.5);
  doc.setTextColor(15, 23, 42);
  doc.text(`Jakarta, ${formatIndonesianShortDate(info.handoverDate)}`, pageW - marginR, sigY, { align: 'right' });
  ...
  doc.setFontSize(9.5);
  doc.setTextColor(15, 23, 42);
  doc.text(info.recipientName, marginL + 5, sigBlockY + 28);
  ...
  doc.setFontSize(9.5);
  doc.setTextColor(15, 23, 42);
  doc.text(info.itSupportName || 'IT Support Dept.', pageW - marginR - colSigW, sigBlockY + 28);
  ...
  doc.save(`BAST-HANDOVER-${asset.assetId}-${cleanName}.pdf`);
}
